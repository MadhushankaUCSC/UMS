...
<body>
	<div class="background">
		<div class="card">
			<form method="post" action="checkvalidity.php" >
				<fieldset>
					...
					<input type="email" name="email" id="email"  placeholder="Enter Email" value="<?php if($_GET['user_email']=='ww'){
						echo '';}else{echo base64_decode($_GET['user_email']) ;} ?>" required=""><!-- input email box-->
					...
					<input type="password" name="password" id="password" placeholder="Enter Password" value="<?php if($_GET['user_email']=='ww'){
						echo '';}else{echo base64_decode($_GET['user_password']) ;} ?>" required=""><!-- input password box-->

					<button  class="btn" type="submit" name="submit" id="submit">Log In</button><br>
				</fieldset>
			</form>
		</div>
	</div>
